...
QUICK DEFENSE VIEW (condensed in-game coach page - what DEF ANALYSIS writes):
    Answers 6 questions in about 10 seconds, comparison-first throughout.
    1. LIVE GAME RUN/PASS  - live snaps, run/pass %, scout %, change
    2. TOP 3 FORMATIONS    - by live usage, run/pass % both sides, change
    3. DOWN/DISTANCE       - 1st/2nd/3rd & Long/Medium/Short (7 rows)
...
STATS (Result-driven player/team stat lines, pulled off both ODK sides):
    1. BALL CARRIER STATS - offense, per player: carries/rush yds/avg,
       receptions/rec yds, rush+rec TD, fumbles, plus team rush/pass yards
    2. QB                 - offense, team-level (no QB name column):
       attempts, completions, comp%, pass yards, pass TD, INT
    3. DEFENSE LIVE YARDS ALLOWED - rushing/passing totals against, live
...
# actually writes). 5 sections, comparison-first: live run/pass, top
# formations, full down & distance, verdict, notable alerts. No top
# run/pass play tables and no live-yards columns - those live on the Stats
# tab now (see build_stats_report below).
# ===========================================================================

def build_quick_runpass_section(identity: IdentityComparison) -> List[Row]:
    """Top-of-page live snap/call information."""
...
def build_quick_down_distance_section(expectations: List[SituationExpectation]) -> List[Row]:
    """Full down & distance table: 1st/2nd/3rd & Long/Medium/Short (7 rows,
    in config.QUICK_DOWN_DISTANCE_LABELS order)."""
    rows: List[Row] = [_section_header("DOWN/DISTANCE")]
    by_label = {e.label: e for e in expectations}
    found = [by_label[w] for w in config.QUICK_DOWN_DISTANCE_LABELS if w in by_label]
    if not found:
...
# ===========================================================================
# STATS - Result-driven player/team stat lines pulled from both ODK sides.
# No football math here either - everything is already computed by
# analysis.build_qb_stats / build_ball_carrier_stats / build_play_type_yards.
# ===========================================================================

def build_stats_ball_carrier_section(
    stats: List[BallCarrierStats],
    team_yards: PlayTypeYards,
) -> List[Row]:
    rows: List[Row] = [_section_header("BALL CARRIER STATS")]
    if not stats:
        rows.append(["(no offensive plays yet)"])
        rows.append(_blank_row())
        return rows

    rows.append(["Ball Carrier", "Carries", "Rush Yds", "Avg/Carry", "Rush TD", "Rec", "Rec Yds", "Rec TD", "Fum"])
    for s in stats:
        rows.append([
            s.ball_carrier,
            str(s.carries),
            _yards(s.rush_yards),
            f"{s.avg_per_carry:.1f}",
            str(s.rush_td),
            str(s.receptions),
            _yards(s.rec_yards),
            str(s.rec_td),
    ...
    rows.append(_blank_row())
    rows.append(["Team Rushing Yards", _yards(team_yards.rushing_yards)])
    rows.append(["Team Passing Yards", _yards(team_yards.passing_yards)])
    rows.append(_blank_row())
    return rows


def build_stats_qb_section(qb: QBStats) -> List[Row]:
    """One blanket QB line - there's no column identifying who the
    quarterback is, so attempts/completions/etc. are team totals."""
    rows: List[Row] = [_section_header("QB")]
    rows.append(["Attempts", "Comp", "Comp%", "Pass Yds", "Pass TD", "INT"])
    rows.append([
        str(qb.attempts),
        str(qb.completions),
        f"{qb.completion_pct:.1f}%",
...
def build_stats_def_yards_section(live_yards: PlayTypeYards) -> List[Row]:
    """Yards the defense has allowed live, moved here from the top of the
    DEF ANALYSIS quick view."""
    rows: List[Row] = [_section_header("DEFENSE LIVE YARDS ALLOWED")]
...
def build_stats_report(
    ...
    off_team_yards: PlayTypeYards,
    qb_stats: QBStats,
    def_live_yards: PlayTypeYards,
) -> List[Row]:
    """Assembles the Stats tab, in order: offense ball carriers, QB, then
    defense live yards allowed."""
    rows: List[Row] = []
    rows += build_stats_ball_carrier_section(ball_carrier_stats, off_team_yards)
...
def build_offense_identity_section(summary: Summary) -> List[Row]:
    """Plays, run/pass split, explosive counts. Ball carrier and team
    yardage now live on the Stats tab (see build_stats_report above)."""
...
def build_offense_report(
    ...
    Ball carrier / team yardage / QB stats live on the Stats tab instead."""
    rows: List[Row] = []
